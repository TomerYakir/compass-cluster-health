...
const ClusterHealthStore = Reflux.createStore({
  ...
  deregisterFromCompass() {
     ...
     global.hadronApp.appRegistry.deregisterAction('ClusterHealth.Actions');
     global.hadronApp.appRegistry.deregisterStore('ClusterHealth.Store');
  },
  ...
  getBalancerEnabled(arg, resolve, reject) {
    console.log("getBalancerEnabled running")
    const sort = [[ '_id', 1 ]];
  ...
  getBalancerRunningByLocks(arg, resolve, reject) {
    ...
    this.dataService.find("config.locks", filter, findOptions, (error, documents) => {
        documents.length == 0 || (documents.length && documents[0].state > 0) ? this.data.balancerRunning = true : this.data.balancerRunning = false;
